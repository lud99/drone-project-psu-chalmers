...
DEFAULT_ACTION_DURATION = 30  # seconds, placeholder
...
class Mission(ABC):
    ...
    def get_tasks(self) -> list:
        '''Returns the list of tasks for this mission, 
        building them if they haven't been built yet.'''
    ...
    def get_frontend_mission_proposal(self) -> dict:
        '''Returns the mission message to be sent to the frontend, 
        which includes the mission parameters and tasks.
        This only returns one single mission and should be appended
...
class NoTypeOrFileError(Exception):
    '''Raised when neither audio_type nor audio_file is specified for a GotoAndAudio mission.'''

class NoAudioFileError(Exception):
    '''Raised when a GotoAndAudio mission cannot find a suitable audio file to play.'''

class MissionBuildError(Exception):
    '''Raised when a mission's tasks cannot be built.'''
...
class GotoAndAudio(Mission):
    ...
    def __init__(
        ...
        if audio_file is None and audio_type is None:
            raise NoTypeOrFileError("Either audio_type or audio_file must be specified.")
    ...
    def get_audio_file(self) -> str:
        '''Returns the audio file to play, based on the specified audio_type or audio_file.'''
        if self.drone.speaker is None:
    ...
    def get_audio_params(self) -> dict:
        '''Returns the audio parameters for this mission (audio_file, duration, volume)'''
        return {
    ...
    def build_tasks(self) -> None:
        '''Builds the list of tasks for the mission'''

        tasks = [
            {
                "action": "go_to",                    
                "params": self.coordinates.as_dict()
            },
            {
                "action": "play_audio",
...
class GotoAndBlink(Mission):
    '''Go to a location and activate its beacon.'''
    def __init__(self, drone: DroneSpecs, coordinates: Coordinates, duration_seconds: int = DEFAULT_ACTION_DURATION):
        super().__init__(drone, coordinates)
        self.duration_seconds = duration_seconds
...
class GotoAndSurveil(Mission):
    '''Go to a location and activate its spotlight.'''
    def __init__(
            self,
            drone: DroneSpecs,
            coordinates: Coordinates,
            duration_seconds: int | None = None,
            camera_pitch: float | None = -90, # In degrees, positive is up. -90 to look down.
            camera_yaw: float | None = 0 # In degrees, positive is to the right. 0 to look straight.
        ):
        super().__init__(drone, coordinates)
        self.duration_seconds = duration_seconds
    ...
    def __repr__(self) -> str:
        return f"GoTo and Surveil with spotlight for {self.duration_seconds}s"
...
class GotoOnly(Mission):
    ...
    def __repr__(self) -> str:
        return f"GoTo, lat:{self.coordinates.lat:.6f}, \
lon:{self.coordinates.lon:.6f} alt:{self.coordinates.alt}"
