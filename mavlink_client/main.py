...
async def main() -> None:
    ...
    receive_loop_task: Optional[asyncio.Task] = None
    ...
    try:
        ...
        async def message_handler(payload: dict) -> None:
            ...
            try:
                # Chalmers task format: {"msg_type": "task", "task_action": {"action": "go_to", "params": {...}}}
                if msg_type == "task":
                    action = payload.get("task_action", {})
                    action_type = action.get("action")
                    params = action.get("params", {})
                    if action_type == "go_to":
                        cmd = Command(type=CommandType.GOTO_POINT, data={
                            "latitude": float(params["lat"]),
                            "longitude": float(params["lon"]),
                            "relative_altitude_m": float(params.get("alt", 15.0)),
                        })
                        result = await controller.execute_command(cmd)
                        print(f"[WS] GoTo result: {result}")
                    elif action_type == "go_home":
                        cmd = Command(type=CommandType.LAND)
                        await controller.execute_command(cmd)

                # Simple goto format: {"msg_type": "goto", "lat": ..., "lon": ..., "alt": ...}
                elif msg_type == "goto":
                    cmd = Command(type=CommandType.GOTO_POINT, data={
                        "latitude": float(payload["lat"]),
                        "longitude": float(payload["lon"]),
                        "relative_altitude_m": float(payload.get("alt", 15.0)),
                    })
                    result = await controller.execute_command(cmd)
                    print(f"[WS] GoTo result: {result}")

                # Mission assignment: {"msg_type": "mission_assignment", "lat": ..., "lon": ..., "alt": ...}
                elif msg_type in ("mission_assignment", "Coordinate_request"):
                    cmd = Command(type=CommandType.GOTO_POINT, data={
                        "latitude": float(payload["lat"]),
                        "longitude": float(payload["lon"]),
                        "relative_altitude_m": float(payload.get("alt", 15.0)),
                    ...
                    await controller.execute_command(Command(type=CommandType.LAND))

                elif msg_type == "abort":
                    await controller.execute_command(Command(type=CommandType.HOLD))

            except Exception as e:
                print(f"[WS] Error handling message: {e}")

        if backend_connected:
            receive_loop_task = asyncio.create_task(
                ws_client.receive_loop(message_handler), name="receive_loop")
        ...
        await safe_cancel(receive_loop_task)
